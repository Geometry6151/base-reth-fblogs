    ...
    ErrorObjectOwned: From<Eth::Error>,
{
    ...
    let result = match result.result {
        ExecutionResult::Success { gas, .. } => {
            FlashblockDryRunResult::success(snapshot_id, gas.tx_gas_used())
        }
        ExecutionResult::Revert { gas, output, .. } => {
            FlashblockDryRunResult::revert(snapshot_id, output, gas.tx_gas_used())
        }
        ExecutionResult::Halt { gas, reason, .. } => {
            FlashblockDryRunResult::halt(snapshot_id, format!("{reason:?}"), gas.tx_gas_used())
        }
    };
